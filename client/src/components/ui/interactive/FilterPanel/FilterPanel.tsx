...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const handleOptionChange = useCallback(
    (
      ...
      if (isRadio) {
        ...
        const sectionValues =
          (newValues[sectionId] as Record<string, boolean>) || {};
        newValues[sectionId] = {
          ...sectionValues,
          [optionId]: checked,
        };
  ...
  const handleSelectChange = useCallback(
    (sectionId: string, value: string | string[], multiple = false) => {
      const newValues = { ...values };
      newValues[sectionId] = value;
      onValuesChange(newValues);
    },
  ...
  const hasActiveFilters = useMemo(() => {
    return sections.some(section => {
      ...
      if (section.type === 'checkbox' || section.type === 'radio') {
        ...
        return (
          range &&
          (range[0] !== section.range.min || range[1] !== section.range.max)
        );
      } else if (section.type === 'select') {
        if (section.multiple) {
  ...
  const renderOptions = useCallback(
    (section: FilterSection) => {
      ...
      const sectionValues =
        (values[section.id] as Record<string, boolean>) || {};
      ...
      const visibleOptions = showMore
        ? section.options
        : section.options.slice(0, maxVisible);
            ...
            return (
              ...
              >
                <input
                  ...
                  aria-describedby={
                    option.count ? `${inputId}-count` : undefined
                  }
                />
                <span className="filter-panel__option-label">
                  {option.label}
                </span>
                {option.count !== undefined && (
                  <span
                    id={`${inputId}-count`}
                    className="filter-panel__option-count"
                  >
                    ({option.count})
                  </span>
                )}
  ...
  const renderRange = useCallback(
    (section: FilterSection) => {
      ...
      const rangeValue = (values[section.id] as [number, number]) || [
        section.range.min,
        section.range.max,
      ];
  ...
  const renderSelect = useCallback(
    (section: FilterSection) => {
      ...
      const selectValue =
        (values[section.id] as string | string[]) ||
        (section.multiple ? [] : '');
      ...
      return (
        <div key={section.id} className="filter-panel__section">
          {section.collapsible !== false ? (
            ...
            >
              <span className="filter-panel__section-title">
                {section.title}
              </span>
              <svg
