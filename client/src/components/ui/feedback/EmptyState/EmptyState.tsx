...
export function EmptyState({
    ...
    switch (variant) {
      case 'search':
        return {
          ...
          description:
            description ||
            "We couldn't find any icon packs matching your search criteria. Try different keywords or browse our categories.",
          icon: (
            <svg
      ...
      case 'favorites':
        return {
          ...
          description:
            description ||
            'Start building your collection by adding icon packs to your favorites.',
          icon: (
            <svg
              className="w-16 h-16 text-gray-400 dark:text-gray-500 mx-auto"
              fill="none"
      ...
      case 'maintenance':
        return {
          ...
          description:
            description ||
            "We're currently updating this section. Please check back shortly.",
          icon: (
            <svg
  ...
  const renderButton = (
    buttonConfig: NonNullable<EmptyStateProps['action']>
  ) => {
    const baseClasses = [
  ...
  return (
    <div className={containerClasses} {...props}>
      ...
      {/* Icon */}
      <div className={sizes.icon}>{finalIcon}</div>

      {/* Content */}
      <div className="space-y-3">
        <h3
          className={`font-semibold text-gray-900 dark:text-white ${sizes.title}`}
        >
          {finalTitle}
        </h3>

        <p
          className={`text-gray-600 dark:text-gray-400 max-w-sm ${sizes.description}`}
        >
          {finalDescription}
        </p>
      </div>
